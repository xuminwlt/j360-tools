...
1. 消费组消费者数量应小于等于队列数量
2. 消费者一次性默认拉取32条消息, 以服务端的模式重现快照,以Map<Long /*偏移量*/, MessageExt> 形式保存TreeMap
3. 流控: 消费数量、消费间隔
...
1. 消费者默认从主服务器brokerId=0处拉取消息
2. 主Broker根据自身情况(可用memory、负载)判断下一次建议拉取的brokerId返回给消费者,多Slave情况则随机选择一个
